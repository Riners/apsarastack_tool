...
//  group_page
    ...
    var upgrade_list_thead = [
        ...
        {"showName":"创建时间","tbodyKey":"create_time","width":"200px"},
//      {"showName":"盘古版本","tbodyKey":"pangu_ver","width":"200px"},
        ...
        {"showName":"BSnapshot","tbodyKey":"PanguSnapshotServer","width":"200px"},
...
//  	if(!$scope.mapsParam.group && !$scope.mapsParam.tag) {
        ...
        data.map(function(item, index) {
        	...
        	var item = item.group_upgrade_list;
            if(item) {
            	tbodyData.push({
            		"index":{
            			"name":index + 1,
            		...
            		"Tag":{
            			"name":title ? title : "",
                        "url":"/upgrade?tag=" + title+"&group="+item.group_name
            		},
            		"group":{
            			"name":item.group_name ? item.group_name : ''
            		},
            		"step":{
                	    "name":((item.total_up_cluster_count/item.total_need_cluster_count)*100).toFixed(3)+ "%" + '\n\r' +'('+item.total_up_cluster_count + '/'+ item.total_need_cluster_count+')'
                    },
                    "create_time":{"name":item.create_time ? item.create_time : '','textAlign':'left'},
//                  "pangu_ver":{
//                  	"name":item.pangu_ver ? item.pangu_ver : ''
//                  },
            		"Master_ver":{"name":item.PanguMaster ? item.PanguMaster : ""},
            		"Chunkserver_ver":{"name":item.PanguChunkserver ? item.PanguChunkserver : ""},
            		"Supervisor_ver":{"name":item.PanguSupervisor ? item.PanguSupervisor : ""},
            		"Monitor_ver":{"name":item.PanguMonitor ? item.PanguMonitor : ""},
            		"Tools_ver":{"name":item.PanguTools ? item.PanguTools : ""},
            		"bm_ver":{"name":item.PanguBlockMaster ? item.PanguBlockMaster : ""},
            		"bs_ver":{"name":item.PanguBlockServer ? item.PanguBlockServer : ""},
            		"bgc_ver":{"name":item.PanguBlockGCWorker ? item.PanguBlockGCWorker : ""},
            		"PanguSnapshotServer":{"name":item.PanguSnapshotServer ? item.PanguSnapshotServer : ""},
            		"owner":{"name":item.owner ? item.owner : ""},
            		"action":{"button":["查看"],"group":title ? title : ''}
            	})
            };
        })
...
//  	}
    ...
    $scope.createUpgradeVer = function(data) {
    	//console.log(data, 'data')
    	$scope.open(data, $scope.upgradeGruopList, $scope.userName)
    }
    //查看非016页面
...
//  				"name":item.upgrade_cfg_desc ? item.upgrade_cfg_desc : ''
//  			};
...
//    	    $scope.getUpgrade_cfg_baseInfo();
	...
	//弹框
    $scope.open = function (item, upgradeGruopList, userName) {
    ...
    $scope.dstep_upgrade_10 = '';
     ...
     $scope.ok = function () {
        ...
        return $http({
            ...
            headers: {'Content-type': 'application/x-www-form-urlencoded;charset=utf-8'},
            data:"tag=" + $scope.tag + "&owner=" + items1.userName + "&group=" + $scope.group + "&PanguMaster_Ver=" + strMsDate + 
                 ...
                 "&PanguBlockGCWorker_Ver="+ PanguBlockGCWorker + "&PanguSnapshotServer_Ver=" + PanguSnapshotServer + strDateParam
        }).then(function(data,status){
        	if(data.data.err) {
        		alert(data.data.err)
        	}else{
